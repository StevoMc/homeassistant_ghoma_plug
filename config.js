...
client.on("message", function (message, remote) {
  ...
  if (initDone) {
    ...
    if (
      (messagesReceive.length === 3 && messagesReceive[0] !== "+ERR=-1") ||
      messagesReceive.length === 4
    ) {
      const mess = messagesReceive[messagesReceive.length - 1];
      const parts = messagesReceive[messagesReceive.length - 1].split(",");
      if (
        parts[parts.length - 1] === controlServer &&
        parts[parts.length - 2] === controlPort + ""
      ) {
    ...
    send(
      new Buffer(
        "AT+NETP=TCP,Client," + controlPort + "," + controlServer + "\r"
      )
    ); // Send new network parameter
